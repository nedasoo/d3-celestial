...
!(function() {
  ...
  var Celestial = {
    ...
    data: [],
    _events:{},
    filesLoaded: Promise.deferred()
  };

  const Collisions = (function(){
    const c = document.createElement('canvas').getContext('2d');
    function measure(text,fontSize,fontFace){
      c.font = fontSize+"px "+fontFace;
      const m = c.measureText(text);
      return [m.fontBoundingBoxAscent+m.fontBoundingBoxDescent,m.width];
    }
    function isColliding(x1,y1,h1,w1,x2,y2,h2,w2){
  ...
  Celestial.on = function(name, listener) {
    if (!this._events[name]) {
      this._events[name] = [];
    }
    this._events[name].push(listener);
  }
  Celestial.removeListener = function(name, listenerToRemove) {
    if (!this._events[name]) {
  ...
  Celestial.emit = function(name, ...data) {
  ...
  Celestial.display = function(config) {
    ...
    if(config.preloadedData) cfg.preloadedData = config.preloadedData;
    ...
    function load() {
      this.filesLoaded = Promise.deferred();
      //Background
      setClip(projectionSetting.clip);
      ...
      const allLoading = ["mw","constellations","constellations_borders","constellations_lines","stars","stars_names","dsos","dsonames","planets"].reduce((a,c)=>{
        a[c] = Promise.deferred();
        return a;
      },{});
  
      //Milky way outline
      new Promise(res=>{
        if(cfg.preloadedData.mw) return res(cfg.preloadedData.mw);
        d3.json(path + "mw.json", function(error, json) {
          Celestial.emit("load","mw",error,json);
          if (error) return allLoading.mw.reject(error),console.warn(error); 
          res(json);
        });
      }).then(json=>{
        allLoading.mw.resolve({type:"mw",json});
      ...
      new Promise(res=>{
        if(cfg.preloadedData.constellations) return res(cfg.preloadedData.constellations);
        d3.json(path + filename("constellations"), function(error, json) {
          Celestial.emit("load","constellations",error,json);
          if (error) return allLoading.constellations.reject(error),console.warn(error);
          res(json);
        });
      }).then(json=>{
        allLoading.constellations.resolve({type:"constellations",json});
      ...
      new Promise(res=>{
        if(cfg.preloadedData.constellations_borders) return res(cfg.preloadedData.constellations_borders);
        d3.json(path + filename("constellations", "borders"), function(error, json) {
          Celestial.emit("load","constellations_borders",error,json);
          if(error) return allLoading.constellations_borders.reject(error),console.warn(error); 
          res(json);
        });
      }).then(json=>{
        allLoading.constellations_borders.resolve({type:"constellations_borders",json});
      ...
      //Constellation lines
      new Promise(res=>{
        if(cfg.preloadedData.constellations_lines) return res(cfg.preloadedData.constellations_lines);
        d3.json(path + filename("constellations", "lines"), function(error, json) {
          Celestial.emit("load","constellations_lines",error,json);
          if(error) return allLoading.constellations_lines.reject(error),console.warn(error);
          res(json);
        });
      }).then(json=>{
        allLoading.constellations_lines.resolve({type:"constellations_lines",json});
      ...
      //Stars
      new Promise(res=>{
        if(cfg.preloadedData.stars) return res(cfg.preloadedData.stars);
        d3.json(path + cfg.stars.data, function(error, json) {
          Celestial.emit("load","stars",error,json);
          if(error) return allLoading.stars.reject(error),console.warn(error);
          res(json);
        });
      }).then(json=>{
        allLoading.stars.resolve({type:"stars",json});
      ...
      //Star names
      new Promise(res=>{
        if(cfg.preloadedData.stars_names) return res(cfg.preloadedData.stars_names);
        d3.json(path + filename("starnames"), function(error, json) {
          Celestial.emit("load","stars_names",error,json);
          if(error) return allLoading.stars_names.reject(error),console.warn(error);
          res(json);
        });
      }).then(json=>{
        allLoading.stars_names.resolve({type:"stars_names",json});
      ...
      //Deep space objects
      new Promise(res=>{
        if(cfg.preloadedData.dsos) return res(cfg.preloadedData.dsos);
        d3.json(path + cfg.dsos.data, function(error, json) {
          Celestial.emit("load","dsos",error,json);
          if (error) return allLoading.dsos.reject(error),console.warn(error);
          res(json);
        })  
      }).then(json=>{
        allLoading.dsos.resolve({type:"dsos",json});
      ...
      //DSO names
      new Promise(res=>{
        if(cfg.preloadedData.dsonames) return res(cfg.preloadedData.dsonames);
        d3.json(path + filename("dsonames"), function(error, json) {
          Celestial.emit("load","dsonames",error,json);
          if (error) return allLoading.dsonames.reject(error),console.warn(error);
          res(json);
        });

      }).then(json=>{
        allLoading.dsonames.resolve({type:"dsonames",json});
      ...
      //Planets, Sun & Moon
      new Promise(res=>{
        if(cfg.preloadedData.planets) return res(cfg.preloadedData.planets);
        d3.json(path + filename("planets"), function(error, json) {
          Celestial.emit("load","planets",error,json);
          if (error) return allLoading.planets.reject(error),console.warn(error);
          res(json);
        });
      }).then(json=>{
        allLoading.planets.resolve({type:"planets",json});
      ...
      Promise.allSettled(Object.values(allLoading)).then(res=>{
        Celestial.filesLoaded.resolve(true);
        Celestial._internalStorage = res.reduce((a,c)=>{
          a[c.value.type] = c.value?.json ?? c.reason;
          return a;
        },{});
        Celestial.emit("load","everything",Celestial._internalStorage);
      });
      //redraw();
    }
    ...
    function redraw() {  
      ...
      if ((cfg.location || cfg.formFields.location) && cfg.planets.show && Celestial.origin) { 
        ...
        container.selectAll(parentElement + " .planet").each(function(d) {
          ...
          if(!cfg.planets.which.includes(id)) return;
          if (clip(pos)) {
  ...
  var settings = { 
    ...
    applyDefaults: function(cfg) {
      ...
      if (cfg) {
        if (has(cfg,"preloadedData")) res.preloadedData = cfg.preloadedData;
  ...
  */
  ...
  function geo(cfg) {
    var dtFormat = d3.time.format("%Y-%m-%d %H:%M:%S"),
        ...
        frm = cfg.form ? d3.select(parentElement + " ~ #celestial-form form").insert("div", "div#general").attr("id", "loc") : null;
  
    var dtpick = cfg.form ? new datetimepicker(config, function(date, tz) { 
    ...
    function go() {
      var lon = geopos[1] ?? parseFloat($form("lon").value),
          lat = geopos[0] ?? parseFloat($form("lat").value),
          tz;
      //Get current configuration
      Object.assign(config, settings.set());
  
      date = date ??dtFormat.parse($form("datetime").value.slice(0,-6));
      ...
      if (!isNaN(lon) && !isNaN(lat)) {
        ...
        if(cfg.form) $form("datetime").value = dateFormat(date, timeZone); 
    ...
    Celestial.skyview = function ({location,date:_date,timezone}) {
      if(!location) throw new Error("invalid location");
      _date = _date ?? new Date();
      date.setTime(_date.valueOf());
  ...
  function exportSVG(fname) {
    ...
    if (cfg.mw.show) {
      q.defer(function(callback) {
        new Promise(resolve=>{
          if(typeof Celestial._internalStorage.mw === "object") resolve(Celestial._internalStorage.mw);
          else{
            d3.json(path + "mw.json", function(error, json) {
    ...
    if (cfg.constellations.bounds) { 
      q.defer(function(callback) { 
        new Promise(resolve=>{
          if(typeof Celestial._internalStorage.constellations_borders === "object") resolve(Celestial._internalStorage.constellations_borders);
          else{
            d3.json(path + filename("constellations", "borders"), function(error, json) {
              if(error) callback(error);
              resolve(json);
    ...
    if (cfg.constellations.lines) { 
      q.defer(function(callback) { 
        new Promise(resolve=>{
          if(typeof Celestial._internalStorage.constellations_lines === "object") resolve(Celestial._internalStorage.constellations_lines);
          else{
            d3.json(path + filename("constellations", "lines"), function(error, json) {
              if(error) callback(error);
              resolve(json);
    ...
    if (cfg.constellations.names) { 
      q.defer(function(callback) { 
        new Promise(resolve=>{
          if(typeof Celestial._internalStorage.constellations === "object") resolve(Celestial._internalStorage.constellations);
          else{
            d3.json(path + filename("constellations"), function(error, json) {
              if(error) callback(error);
              resolve(json);
    ...
    if (cfg.stars.show) { 
      q.defer(function(callback) { 
        new Promise(resolve=>{
          if(typeof Celestial._internalStorage.stars === "object") resolve(Celestial._internalStorage.stars);
          else{
            d3.json(path +  cfg.stars.data, function(error, json) {
              if(error) callback(error);
              resolve(json);
    ...
    if (cfg.dsos.show) { 
      q.defer(function(callback) { 
        new Promise(resolve=>{
          if(typeof Celestial._internalStorage.dsos === "object") resolve(Celestial._internalStorage.dsos);
          else{
            d3.json(path +  cfg.dsos.data, function(error, json) {
              if(error) callback(error);
              resolve(json);
      ...
      q.defer(function(callback) {
        ...
        if (cfg.planets.symbolType === "disk") {
          groups.planets.selectAll(".planets")
           ...
           .attr("transform", function(d) { return point(d.geometry.coordinates); })
           .attr("d", function(d) { 
             var r = d.properties.radius;//(has(cfg.planets.symbols[d.id], "size")) ? (cfg.planets.symbols[d.id].size - 1) * adapt : null;
             //console.log(d);
        ...
        if (cfg.planets.names) {
          const usedSpace = [];
          const [planetFontSize,planetFontFace] = styles.planetNames.font.split("px ");
          groups.planetNames.selectAll(".planetnames")
          .data(jp.features)
          .enter().append("text")
          .attr("transform", function(d) {
            const [x,y] = projection(d.geometry.coordinates);
            const [textHeight,textWidth] = Collisions.measure(d.properties.name,planetFontSize,planetFontFace);
            let dx=0;
            let dy = d.properties.radius + 5;
            for(let uspace of usedSpace){
              if(Collisions.isColliding(uspace.x,uspace.y,uspace.textHeight,uspace.textWidth,x,y,textHeight,textWidth)){
                dy = -dy;
                break;
              }
            }
    ...
    function svgTextStyle(s) {
      ...
      res["alignment-baseline"] = s.baseline || "auto";
      res.font = s.font || null;
      return res;
    }
    ...
    function createEntry(o) {
      ...
      res.properties.radius = has(cfg.planets.symbols[res.id], "size") ? (cfg.planets.symbols[res.id].size - 1) * adapt : planetSize(res.properties.mag);
      if (res.id === "lun") {
